...
module.exports = BtceManager;

function BtceManager(filePath, callback) {
	this.filePath = filePath;
	this.createKeyHandlers(callback);
}


BtceManager.prototype.createKeyHandlers = function(callback) {
	if (fs.existsSync(this.filePath)) {
		var data = JSON.parse(fs.readFileSync("config.json"));
		callback(null, data);
	} else {
		callback('No such file exist : '+this.filePath, null);
	}
};
